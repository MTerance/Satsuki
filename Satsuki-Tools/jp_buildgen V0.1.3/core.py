...
def _add_cube(name, size=(2,2,2), loc=(0,0,0), mat=None):
    ...
    # Entrer en mode édition pour déplacer les vertices
    bpy.context.view_layer.objects.active = o
    bpy.ops.object.mode_set(mode='EDIT')
    
    # Sélectionner tous les vertices et les déplacer vers le haut
    bpy.ops.mesh.select_all(action='SELECT')
    # Déplacer de la moitié de la hauteur vers le haut (en coordonnées locales)
    ...
    # La base du cube est maintenant à Z=0 en coordonnées locales
    o.location = loc
...
def _add_plane(name, size=(2,2), loc=(0,0,0), mat=None):
    # Créer un plan à l'origine
    bpy.ops.mesh.primitive_plane_add(size=1.0, location=loc)
    o = bpy.context.active_object
    o.name = name
    
    # Appliquer la taille (seulement X et Y pour un plan)
...
def _build_restaurant(root, rng, foot_x, foot_y, floors, fh, M):
    ...
    # Auvent à hauteur fixe
    _add_cube(PREFIX+"Rest_Awning", (foot_x*0.9, 0.06, 0.08), (0, (foot_y*0.8)/2 + 0.03, GROUND_LEVEL + 2.1), M["sign"]).parent = root
...
def _build_house(root, rng, foot_x, foot_y, floors, fh, M):
    ...
    # Porche posé sur le sol
    _add_cube(PREFIX+"House_Porch", (w*0.5, 0.6, 0.2), (0, d/2 + 0.30, GROUND_LEVEL + 1.6), M["roof"]).parent = root
