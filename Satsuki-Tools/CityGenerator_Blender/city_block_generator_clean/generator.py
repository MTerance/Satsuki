...
City Block Generator - Version Minimale Fonctionnelle

def generate_city(context, props):

    """Génère une ville procédurale"""Génération de villes avec système de variété de couleursCity Block Generator - Version Minimale Fonctionnelleimport random

    print("Génération de ville...")

    """

    width = props.get('width', 3)

    length = props.get('length', 3)Génération basique de villes avec améliorations de variétéimport traceback

    max_floors = props.get('max_floors', 6)

    road_width = props.get('road_width', 4.0)import bpy

    buildings_per_block = props.get('buildings_per_block', 3)

    building_variety = props.get('building_variety', 'MEDIUM')import random"""
    ...
    def generate_city(context, props):# Compteur global pour les bâtiments

    for x in range(width):

        for y in range(length):    """Fonction principale de génération de ville"""

            generate_block(context, collection, x, y, road_width, max_floors, buildings_per_block, building_variety)

        print("🏙️ Génération de ville avec variété...")import bpybuilding_counter = 0

    print(f"Ville générée: {width}x{length}, variété {building_variety}")

    

def generate_block(context, collection, x, y, road_width, max_floors, buildings_per_block, variety):

    """Génère un bloc de ville"""    # Récupération des paramètresimport random

    block_size = 20.0

    spacing = block_size + road_width    width = props.get('width', 3)

    base_x = x * spacing

    base_y = y * spacing    length = props.get('length', 3)import math# Validation des paramètres

    

    for i in range(buildings_per_block):    max_floors = props.get('max_floors', 6)

        offset_x = random.uniform(2, block_size - 8)

        offset_y = random.uniform(2, block_size - 8)    road_width = props.get('road_width', 4.0)def validate_parameters(width, length, max_floors):

        pos_x = base_x + offset_x

        pos_y = base_y + offset_y    buildings_per_block = props.get('buildings_per_block', 3)

        

        floors = random.randint(max(2, max_floors - 4), max_floors)    building_variety = props.get('building_variety', 'MEDIUM')def generate_city(context, props):    """Valide les paramètres d'entrée pour la génération de ville"""

        height = floors * 3.0

        size_x = random.uniform(4, 8)    

        size_y = random.uniform(4, 8)

            print(f"   Dimensions: {width}x{length}")    """Fonction principale de génération de ville"""    errors = []

        bpy.ops.mesh.primitive_cube_add(size=1, location=(pos_x, pos_y, height / 2))

        building = context.active_object    print(f"   Variété: {building_variety}")

        building.scale = (size_x, size_y, height)

        building.name = f"Building_{x}_{y}_{i}"        print("🏙️ Génération de ville avec variété...")    

        

        if building.name in context.scene.collection.objects:    # Créer une collection pour la ville

            context.scene.collection.objects.unlink(building)

        collection.objects.link(building)    collection_name = f"City_{width}x{length}"        if width < 1 or width > 50:

        

        apply_varied_material(building, variety)    collection = bpy.data.collections.new(collection_name)



def apply_varied_material(obj, variety):    context.scene.collection.children.link(collection)    # Paramètres        errors.append(f"Largeur invalide: {width}. Doit être entre 1 et 50.")

    """Applique un matériau coloré"""

    if variety == 'LOW':    

        colors = [(0.8, 0.8, 0.8, 1.0), (0.7, 0.7, 0.7, 1.0), (0.9, 0.9, 0.9, 1.0)]

    elif variety == 'MEDIUM':    # Générer les blocs    width = props.get('width', 3)    

        colors = [(0.85, 0.82, 0.75, 1.0), (0.75, 0.75, 0.75, 1.0), (0.65, 0.72, 0.78, 1.0), 

                  (0.82, 0.80, 0.75, 1.0), (0.70, 0.70, 0.70, 1.0), (0.60, 0.60, 0.60, 1.0)]    block_count = 0

    elif variety == 'HIGH':

        colors = [(0.85, 0.82, 0.75, 1.0), (0.65, 0.72, 0.78, 1.0), (0.75, 0.78, 0.70, 1.0),     for x in range(width):    length = props.get('length', 3)    if length < 1 or length > 50:

                  (0.82, 0.75, 0.72, 1.0), (0.70, 0.72, 0.75, 1.0), (0.55, 0.62, 0.70, 1.0), 

                  (0.45, 0.45, 0.45, 1.0), (0.65, 0.55, 0.48, 1.0)]        for y in range(length):

    else:

        colors = [(0.85, 0.82, 0.75, 1.0), (0.95, 0.95, 0.90, 1.0), (0.82, 0.80, 0.75, 1.0),             generate_block(    max_floors = props.get('max_floors', 6)        errors.append(f"Longueur invalide: {length}. Doit être entre 1 et 50.")

                  (0.88, 0.88, 0.88, 1.0), (0.65, 0.72, 0.78, 1.0), (0.75, 0.78, 0.70, 1.0), 

                  (0.82, 0.75, 0.72, 1.0), (0.70, 0.72, 0.75, 1.0), (0.55, 0.62, 0.70, 1.0),                 context, collection, x, y,

                  (0.40, 0.50, 0.55, 1.0), (0.25, 0.25, 0.25, 1.0), (0.55, 0.48, 0.38, 1.0), 

                  (0.75, 0.75, 0.75, 1.0), (0.45, 0.45, 0.45, 1.0), (0.65, 0.55, 0.48, 1.0),                 road_width, max_floors,    road_width = props.get('road_width', 4.0)        

                  (0.62, 0.62, 0.60, 1.0), (0.48, 0.52, 0.48, 1.0), (0.65, 0.40, 0.35, 1.0)]

                    buildings_per_block, building_variety

    color = random.choice(colors)

    mat = bpy.data.materials.new(name=f"BuildingMat_{obj.name}")            )    buildings_per_block = props.get('buildings_per_block', 3)    if max_floors < 1 or max_floors > 100:
    ...
    if bsdf:        building_variety = props.get('building_variety', 'MEDIUM')        errors.append(f"Nombre d'étages invalide: {max_floors}. Doit être entre 1 et 100.")

        bsdf.inputs['Base Color'].default_value = color

        bsdf.inputs['Roughness'].default_value = random.uniform(0.3, 0.7)    print(f"✅ Ville générée: {block_count} blocs, variété {building_variety}")

    

    if obj.data.materials:    return collection        
...
def generate_block(context, collection, x, y, road_width, max_floors, buildings_per_block, variety):    # Créer la collection    # Vérifier si nous avons trop de blocs (limite de performance)

print("Module generator chargé")

    """Génère un bloc de ville avec bâtiments"""

        collection = bpy.data.collections.new(f"City_{width}x{length}")    total_blocks = width * length

    # Paramètres du bloc

    block_size = 20.0    context.scene.collection.children.link(collection)    if total_blocks > 100:

    spacing = block_size + road_width

                errors.append(f"Trop de blocs: {total_blocks}. Maximum recommandé: 100 (10x10).")

    base_x = x * spacing

    base_y = y * spacing    # Générer les blocs    

    

    # Générer les bâtiments dans le bloc    for x in range(width):    return errors

    for i in range(buildings_per_block):

        # Position aléatoire dans le bloc        for y in range(length):

        offset_x = random.uniform(2, block_size - 8)

        offset_y = random.uniform(2, block_size - 8)            generate_block(def check_blender_state():

        

        pos_x = base_x + offset_x                context, collection, x, y,    """Vérifie que Blender est dans un état approprié pour la génération"""

        pos_y = base_y + offset_y

                        road_width, max_floors,    try:

        # Hauteur variable

        floors = random.randint(max(2, max_floors - 4), max_floors)                buildings_per_block, building_variety        # Vérifier que nous avons accès au contexte
        ...
        size_x = random.uniform(4, 8)                return ["Contexte Blender non disponible."]

        size_y = random.uniform(4, 8)

            print(f"✅ Ville {width}x{length} générée avec variété {building_variety}")        

        # Créer le bâtiment (cube de base)

        bpy.ops.mesh.primitive_cube_add(        # Vérifier que nous avons une scène active

            size=1,

            location=(pos_x, pos_y, height / 2)def generate_block(context, collection, x, y, road_width, max_floors, buildings_per_block, variety):        if not bpy.context.scene:

        )

            """Génère un bloc de ville"""            return ["Aucune scène active dans Blender."]

        building = context.active_object

        building.scale = (size_x, size_y, height)    block_size = 20.0            

        building.name = f"Building_{x}_{y}_{i}"

            spacing = block_size + road_width        # Vérifier que nous sommes en mode Object

        # Ajouter à la collection

        if building.name in context.scene.collection.objects:            if bpy.context.mode != 'OBJECT':

            context.scene.collection.objects.unlink(building)

        collection.objects.link(building)    base_x = x * spacing            return ["Blender doit être en mode Object pour la génération."]

        

        # Appliquer un matériau avec couleur    base_y = y * spacing            

        apply_varied_material(building, variety)

            return []

def apply_varied_material(obj, variety):

    """Applique un matériau coloré selon le niveau de variété"""    # Générer les bâtiments    except Exception as e:

    

    # Palettes de couleurs selon le niveau    for i in range(buildings_per_block):        return [f"Erreur lors de la vérification de l'état Blender: {str(e)}"]

    if variety == 'LOW':

        colors = [        offset_x = random.uniform(2, block_size - 8)

            (0.8, 0.8, 0.8, 1.0),

            (0.7, 0.7, 0.7, 1.0),        offset_y = random.uniform(2, block_size - 8)# Fonction utilitaire pour convertir en toute sécurité les propriétés Blender en nombres
        ...
        colors = [        pos_x = base_x + offset_x    """Convertit une valeur en float de façon sécurisée"""

            (0.85, 0.82, 0.75, 1.0),

            (0.75, 0.75, 0.75, 1.0),        pos_y = base_y + offset_y    try:

            (0.65, 0.72, 0.78, 1.0),

            (0.82, 0.80, 0.75, 1.0),                # Essayer d'accéder directement comme valeur numérique

            (0.70, 0.70, 0.70, 1.0),

            (0.60, 0.60, 0.60, 1.0)        # Hauteur variable        return float(value)

        ]

    elif variety == 'HIGH':        floors = random.randint(max(2, max_floors - 4), max_floors)    except (TypeError, ValueError):

        colors = [

            (0.85, 0.82, 0.75, 1.0),        height = floors * 3.0        try:

            (0.65, 0.72, 0.78, 1.0),

            (0.75, 0.78, 0.70, 1.0),                    # Si c'est une PropertyDeferred, essayer d'obtenir sa valeur

            (0.82, 0.75, 0.72, 1.0),

            (0.70, 0.72, 0.75, 1.0),        # Taille variable            return float(value.real)

            (0.55, 0.62, 0.70, 1.0),

            (0.45, 0.45, 0.45, 1.0),        size_x = random.uniform(4, 8)        except (AttributeError, TypeError, ValueError):

            (0.65, 0.55, 0.48, 1.0)

        ]        size_y = random.uniform(4, 8)            try:

    else:  # EXTREME

        colors = [                        # Comme dernier recours, essayer de le convertir en string puis en float

            (0.85, 0.82, 0.75, 1.0),

            (0.95, 0.95, 0.90, 1.0),        # Créer le bâtiment                return float(str(value))

            (0.82, 0.80, 0.75, 1.0),

            (0.88, 0.88, 0.88, 1.0),        bpy.ops.mesh.primitive_cube_add(            except (TypeError, ValueError):

            (0.65, 0.72, 0.78, 1.0),

            (0.75, 0.78, 0.70, 1.0),            size=1,                # En cas d'échec complet, retourner la valeur par défaut

            (0.82, 0.75, 0.72, 1.0),

            (0.70, 0.72, 0.75, 1.0),            location=(pos_x, pos_y, height / 2)                print(f"Conversion en float échouée, utilisation de la valeur par défaut: {default}")

            (0.55, 0.62, 0.70, 1.0),

            (0.40, 0.50, 0.55, 1.0),        )                return default
            ...
            (0.45, 0.45, 0.45, 1.0),        building = context.active_objectdef safe_int(value, default=0):

            (0.65, 0.55, 0.48, 1.0),

            (0.62, 0.62, 0.60, 1.0),        building.scale = (size_x, size_y, height)    try:

            (0.48, 0.52, 0.48, 1.0),

            (0.65, 0.40, 0.35, 1.0)        building.name = f"Building_{x}_{y}_{i}"        # Essayer d'accéder directement comme valeur numérique
    ...
    color = random.choice(colors)        # Ajouter à la collection    except (TypeError, ValueError):

    

    # Créer le matériau        if building.name in context.scene.collection.objects:        try:

    mat = bpy.data.materials.new(name=f"BuildingMat_{obj.name}")

    mat.use_nodes = True            context.scene.collection.objects.unlink(building)            # Si c'est une PropertyDeferred, essayer d'obtenir sa valeur

    

    # Configurer le noeud Principled BSDF        collection.objects.link(building)            return int(value.real)

    bsdf = mat.node_tree.nodes.get("Principled BSDF")

    if bsdf:                except (AttributeError, TypeError, ValueError):

        bsdf.inputs['Base Color'].default_value = color

        bsdf.inputs['Roughness'].default_value = random.uniform(0.3, 0.7)        # Appliquer un matériau avec couleur            try:

    

    # Appliquer le matériau à l'objet        apply_varied_material(building, variety)                # Comme dernier recours, essayer de le convertir en string puis en int

    if obj.data.materials:

        obj.data.materials[0] = mat                return int(str(value))

    else:

        obj.data.materials.append(mat)def apply_varied_material(obj, variety):            except (TypeError, ValueError):



# Message de chargement    """Applique un matériau coloré selon le niveau de variété"""                # En cas d'échec complet, retourner la valeur par défaut

print("✅ Module generator chargé - Version minimale fonctionnelle")

    # Palettes de couleurs                print(f"Conversion en entier échouée, utilisation de la valeur par défaut: {default}")

    if variety == 'LOW':                return default

        colors = [

            (0.8, 0.8, 0.8, 1.0),  # Gris clairdef create_material(name, color):

            (0.7, 0.7, 0.7, 1.0),  # Gris    """Crée un matériau avec gestion d'erreurs et support des Shader Nodes"""

            (0.9, 0.9, 0.9, 1.0),  # Blanc cassé    try:

        ]        mat = bpy.data.materials.get(name)

    elif variety == 'MEDIUM':        if not mat:

        colors = [            mat = bpy.data.materials.new(name)

            (0.85, 0.82, 0.75, 1.0),  # Beige            # Activer les nodes pour Blender moderne

            (0.75, 0.75, 0.75, 1.0),  # Gris            mat.use_nodes = True

            (0.65, 0.72, 0.78, 1.0),  # Bleu-gris            

            (0.82, 0.80, 0.75, 1.0),  # Crème            # Obtenir le node Principled BSDF

            (0.70, 0.70, 0.70, 1.0),  # Gris moyen            if mat.node_tree:

            (0.60, 0.60, 0.60, 1.0),  # Gris foncé                principled = mat.node_tree.nodes.get("Principled BSDF")

        ]                if principled:

    elif variety == 'HIGH':                    # Définir la couleur de base (Base Color)

        colors = [                    principled.inputs[0].default_value = (*color, 1.0)

            (0.85, 0.82, 0.75, 1.0),  # Beige                    print(f"Matériau '{name}' créé avec couleur {color}")

            (0.65, 0.72, 0.78, 1.0),  # Bleu-gris                else:

            (0.75, 0.78, 0.70, 1.0),  # Vert sage                    print(f"Node Principled BSDF introuvable pour {name}")

            (0.82, 0.75, 0.72, 1.0),  # Rose poudré            

            (0.70, 0.72, 0.75, 1.0),  # Gris moderne            # Fallback pour compatibilité

            (0.55, 0.62, 0.70, 1.0),  # Bleu corporate            mat.diffuse_color = (*color, 1.0)

            (0.45, 0.45, 0.45, 1.0),  # Gris métal        else:

            (0.65, 0.55, 0.48, 1.0),  # Rouille            print(f"Matériau '{name}' existe déjà, mise à jour de la couleur")

        ]            # Mettre à jour la couleur même si le matériau existe

    else:  # EXTREME            if mat.use_nodes and mat.node_tree:

        colors = [                principled = mat.node_tree.nodes.get("Principled BSDF")

            (0.85, 0.82, 0.75, 1.0),  # Beige                if principled:

            (0.95, 0.95, 0.90, 1.0),  # Blanc cassé                    principled.inputs[0].default_value = (*color, 1.0)

            (0.82, 0.80, 0.75, 1.0),  # Crème            mat.diffuse_color = (*color, 1.0)

            (0.88, 0.88, 0.88, 1.0),  # Gris clair            

            (0.65, 0.72, 0.78, 1.0),  # Bleu-gris        return mat

            (0.75, 0.78, 0.70, 1.0),  # Vert sage    except Exception as e:

            (0.82, 0.75, 0.72, 1.0),  # Rose poudré        print(f"Erreur lors de la création du matériau '{name}': {str(e)}")

            (0.70, 0.72, 0.75, 1.0),  # Gris moderne        # Retourner un matériau par défaut ou None

            (0.55, 0.62, 0.70, 1.0),  # Bleu corporate        try:

            (0.40, 0.50, 0.55, 1.0),  # Verre teinté            # Essayer de créer un matériau de base

            (0.25, 0.25, 0.25, 1.0),  # Noir élégant            default_mat = bpy.data.materials.new(f"default_{name}")

            (0.55, 0.48, 0.38, 1.0),  # Bronze            default_mat.use_nodes = True

            (0.75, 0.75, 0.75, 1.0),  # Silver            if default_mat.node_tree:

            (0.45, 0.45, 0.45, 1.0),  # Gris métal                principled = default_mat.node_tree.nodes.get("Principled BSDF")

            (0.65, 0.55, 0.48, 1.0),  # Rouille                if principled:

            (0.62, 0.62, 0.60, 1.0),  # Béton                    principled.inputs[0].default_value = (0.5, 0.5, 0.5, 1.0)

            (0.48, 0.52, 0.48, 1.0),  # Vert militaire            default_mat.diffuse_color = (0.5, 0.5, 0.5, 1.0)

            (0.65, 0.40, 0.35, 1.0),  # Rouge brique            return default_mat
    ...
    color = random.choice(colors)def create_varied_material(building_index, zone_type='RESIDENTIAL', building_type='rectangular'):

        """Crée un matériau varié selon le type de bâtiment et la zone"""

    # Créer le matériau    import random

    mat = bpy.data.materials.new(name=f"BuildingMat_{obj.name}")    

    mat.use_nodes = True    # Palettes de couleurs par zone

        color_palettes = {

    # Configurer le noeud Principled BSDF        'RESIDENTIAL': [

    bsdf = mat.node_tree.nodes.get("Principled BSDF")            (0.9, 0.85, 0.8),   # Beige clair

    if bsdf:            (0.8, 0.7, 0.6),    # Terre cuite

        bsdf.inputs['Base Color'].default_value = color            (0.7, 0.8, 0.9),    # Bleu pastel

        bsdf.inputs['Roughness'].default_value = random.uniform(0.3, 0.7)            (0.9, 0.9, 0.8),    # Crème

                (0.8, 0.85, 0.7),   # Vert olive clair

    # Appliquer le matériau            (0.9, 0.8, 0.7),    # Saumon

    if obj.data.materials:            (0.85, 0.8, 0.9),   # Lavande

        obj.data.materials[0] = mat        ],

    else:        'COMMERCIAL': [

        obj.data.materials.append(mat)            (0.2, 0.3, 0.8),    # Bleu corporate

            (0.8, 0.8, 0.8),    # Gris métallique

print("✅ Module generator chargé (version minimale fonctionnelle)")            (0.9, 0.9, 0.9),    # Blanc moderne
