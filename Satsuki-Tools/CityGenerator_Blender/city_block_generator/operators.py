...
def get_city_properties():
    ...
    return {
        'citygen_width': bpy.props.IntProperty(
            name="Largeur",
            description="Nombre de blocs en largeur (LIMITE: max 5 pour éviter crashes)",
            default=3,  # Réduit pour sécurité
            min=1,
            max=5  # Limite drastique pour éviter crashes
        ),
        'citygen_length': bpy.props.IntProperty(
            name="Longueur", 
            description="Nombre de blocs en longueur (LIMITE: max 5 pour éviter crashes)",
            default=3,  # Réduit pour sécurité
            min=1,
            max=5  # Limite drastique pour éviter crashes
        ),
        'citygen_max_floors': bpy.props.IntProperty(
            name="Étages Maximum",
            description="Nombre maximum d'étages pour les bâtiments (LIMITE: max 15 pour performance)",
            default=5,  # Réduit pour sécurité
            min=1,
            max=15  # Limite pour éviter les bâtiments trop complexes
        ),
        'citygen_road_width': bpy.props.FloatProperty(
            ...
            default=4.0,
            min=1.0,
            max=8.0  # Réduit légèrement
        ),
        'citygen_buildings_per_block': bpy.props.IntProperty(
            name="Bâtiments par Bloc",
            description="Nombre de bâtiments par bloc (LIMITE: max 2 pour performance)",
            default=1,  # Réduit pour sécurité
            min=1,
            max=2  # Limite drastique pour performance
        ),
